...
**Dodatkowo:** nauczysz się podstawowej obsługi terminala systemu Linux – poruszania się po katalogach, kopiowania plików, uruchamiania programów z linii komend.
...
```
ssh biotech25@150.254.120.213
```

- `ssh` (*secure shell*) to sposób łączenia się ze zdalnym komputerem (serwerem), na którym będziemy pracowali.
- `biotech25` to nazwa użytkownika.
- `150.254.120.213` to adres maszyny, z którą się łączymy.

Hasło udostępnione jest na sali ćwiczeń.
...
To aktywuje specjalne środowisko, w którym zainstalowane są wszystkie potrzebne narzędzia: `bwa`, `samtools`, `bcftools`, itp.
...
Korzystając z `mkdir` (*make directory*) stwórz swój katalog roboczy nazywając go zgodnie ze swoim uniwersyteckim ID (**TWOJE_ID**@st.amu.edu.pl).
...
3. Wpisz adres: `sftp://biotech25@150.254.120.213`
4. Wprowadź hasło
...
**Zadanie:** korzystając z polecenia `du -h plik` (*disk usage*) określ, który z plików - w formacie *sam*, czy *bam* jest większy? Zapisz ich wielkość. 
